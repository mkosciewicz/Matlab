...
%Initialize the arduino Uno and Servo
clc;clear;close;
% a = webcam('/dev/video0'); %Initialize arduino at /dev/ttyACM0
% s = servo(a, 'D3');                   %Initialize servo @ Digitial pin 3 of arduino Uno
% writePosition(s,0.5);             %writePosition(servo_name,angle)   0<=angle<=1;            
...
%Initially we are initialzing the Live video stream
  ...
  vid = ipcam('http://192.168.0.225:8080/video');
  %videoinput('winvideo','1', 'RGB24_320X240');  %videoinput('apaptername','device_ID','format')
...
if((R>G) && (R>B))                         %if the area of Red is greater than Green and Blue then its Red
    fprintf('RED');
    writePosition(s,0.25);                  %Writes servo position as 0.3*180= 54 degrees
